...
- 标题前只读紧凑标签显示“未标记 / Draft / Bad case / 纳入 Eval”；未标记表示尚无样本。标签不增加菜单，原有 Thread、messages 和编辑入口保留。
- Eval 人/时间取最近一次人工成功保存；由 Web Session 和服务端时钟绑定，在悬浮信息与编辑器按需查看。旧样本缺失信息显示未记录，不从创建或更新时间推断。

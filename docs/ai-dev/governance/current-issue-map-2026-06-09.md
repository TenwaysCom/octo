...
| P2 | 文档/测试/运行时代码对 legacy route 的说法不一致 | 影响 API 兼容性判断和 PR review | 明确兼容策略并同步 AGENTS/docs/tests |
...
1. 每次 action 执行生成 `actionRunId`，贯穿 extension、background、server、adapter。
2. 所有 action 失败统一返回 error envelope：
...
- Server API error response 保留 `layer/module/stage/actionRunId`。
...
## P2-1. Compatibility Route Policy Is Inconsistent

### Problem

当前项目说明里仍提到旧 `/api/a1/*`、`/api/a2/*` compatibility aliases 保留，但代码和测试已经认为 legacy routes 不再注册。
...
- Current project instructions still describe old `/api/a1/*` and `/api/a2/*` routes as compatibility aliases.

### Impact

- 开发者不确定新业务要接 `/api/lark-base/*`、`/api/lark-bug/*` 还是旧 alias。
- PR review 对 route 删除/保留的判断没有统一依据。
- 外部使用方如果仍依赖旧 route，风险不清。

### Recommended Direction

做一次 route policy 决策：

1. 如果旧 route 已正式退役，更新 AGENTS/project docs，明确不再保留兼容 alias。
2. 如果仍需兼容，恢复 route 并加 deprecation warning/test。
3. 把 public API route policy 放进 `docs/ai-dev` 的 rules 或 governance 文档。

### Acceptance Criteria

- AGENTS、docs、`index.test.ts`、runtime route 注册对 legacy routes 的说法一致。
- 如果保留 alias，有测试证明 alias 和 canonical route 行为一致。
- 如果不保留 alias，有迁移说明或风险记录。
...
### Step 4: Align Docs And Route Policy

目标：让 AGENTS、docs、tests、runtime route 注册一致。

交付：

- 明确 legacy A1/A2/lark-bug/lark-user-story route 是否保留。
- 更新相应 docs 和 tests。

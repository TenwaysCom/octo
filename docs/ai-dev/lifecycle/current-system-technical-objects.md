...
| `MeegleWorkitemLifecycleSnapshot` | Server projection / PostgreSQL | `server/src/application/services/meegle-workitem-lifecycle.ts`, `server/src/adapters/postgres/platform-sync-store.ts` | 保存当前 Sprint、精确 current-node/add-to-cycle 时间及源端日粒度 start/finish 投影；来源清空覆盖旧值 |
...
| `SupportKnowledgeDocument` / `SupportKnowledgeChunk` | Server PostgreSQL store | `server/src/adapters/postgres/support-knowledge-store.ts`, `server/src/application/services/lark-ticket-ai-session.service.ts` | 仅保存脱敏内容；只有人工批准的受控文档和历史案例可被 Ticket Answer Quick Action 以 `source_ref` 引用检索 |
| `SupportTicketAnalysis` | Server application service / PostgreSQL projection | `server/src/application/services/support-ticket-analysis.service.ts`, `server/src/adapters/postgres/support-ticket-analysis-store.ts`, `server/src/modules/lark-ticket/lark-ticket.controller.ts` | intent、result、quality 绑定固定 Ticket snapshot；Web PUT 负责人工更新；Summary Quick Action 与 Shadow 共用完整消息选取/精简及 M 短引用；输入含省略范围和快照新鲜度，Server 校验实际呈现证据并回映射长 ID，再经 JSON schema、snapshot version 门禁更新规范化分析表及 FE `ticket_ai` 投影。失败时不得写入正式分析投影。 |
...
| `TicketSummaryQuickAction` | Server workflow / Ticket Summary adapter | `server/src/application/services/lark-ticket-ai-session.service.ts`, `server/src/adapters/ai/ticket-summary-client.ts`, `server/src/domain/workflow-prompts.ts` | `lark-ticket-support-qa-summarize` 的一次性结构化分析；不启动 ACP、不创建可续聊 Session、不暴露 workspace 或工具权限。复用浏览器 SSE 外壳展示当前结果，长期结果由 `SupportTicketAnalysis` 投影承载。thread 输入与 Shadow 共用 60,000 字符完整块预算、短引用与堆栈精简；保留独立正式提示词和结果结构，运行时补充引用契约以兼容数据库自定义模板。 |
| `TicketWikiQaQuickAction` / `WikiKnowledgeEvidence` | Server workflow / filesystem reader / completion clients | `server/src/application/services/wiki-qa.service.ts`, `server/src/adapters/filesystem/wiki-knowledge-reader.ts`, `server/src/adapters/ai/wiki-qa-rerank-client.ts` | `lark-ticket-wiki-qa` 获取固定 Ticket 聊天快照，该流程的模型上下文使用与 Shadow 共用的 M1/M2 消息短标签并转换回复引用，移除重复 ID 列表；原快照保留长 ID，提取问题（关键词逐项校验后trim、忽略大小写去重，按原顺序保留前20项）、本地召回默认最多20篇 wiki 页面（`WIKI_QA_RECALL_LIMIT` 可配1–100，与 Shadow 共用），全部候选交给独立重排模型（general 保留证据核验输入；专用 rerank 仅发送标题、环境和相关 Wiki 内容，不带历史聊天），重排返回最多5篇 FAQ / QA Card，专用模式先按 `WIKI_QA_RERANK_MIN_SCORE`（默认0.7，含等号）过滤，再按核心错误签名或有效主题过滤，按排名取最多3篇供答案使用（第4/5名可补位），可保留0篇并重新连续编号；英文停用词/单词边界匹配，省略知识维护与结论模板。答案另用字段白名单及精简栈帧上下文，保留cause与消息回复关系；问题提取和答案共用 `WIKI_QA_ANSWER_REASONING_EFFORT` 请求low（可配置high/max/provider，仅ZCode的glm-5.3系列发送），日志增加字符数、可用token及响应头/总耗时；index/entities 仅导航，raw 仅提供脱敏原始证据，Shadow AI 不作为原始证据。Server 分配引用编号，保留草稿与适用性边界，提取和回答仍使用共享问题总结模型；重排使用 `WIKI_QA_RERANK_MODE=general|rerank`：默认general为智谱 `glm-5.3-flash` Chat Completions；rerank为显式URL/model的专用query/documents协议，校验索引/分数后映射证据，保守标为历史参考/前提未核验。Reader 在候选对象关联的 WeakMap 中保留原始来源，重排入选后优先沿相关知识来源链接，按问题词、相邻消息及前提/结果/纠正规则选取原文片段；保留来源路径及源内 M/P 序号，整组上下文超预算时省略并降级，不截掉后续否定，不生成冒充原文的摘要。URL必须完整。现有超时不收紧；仅general默认地址可复用 `ZCODE_API_KEY`。提取阶段可通过 `WIKI_QA_EXTRACT_LOG_ENABLED=true` 开启专用按日日志（默认关闭），保存脱敏 prompt/输出及校验状态，以 actionRunId 关联；重排通过独立 `WIKI_QA_RERANK_LOG_ENABLED=true` 记录两种模式的脱敏请求/响应，默认关闭；答案阶段使用独立 `WIKI_QA_ANSWER_LOG_ENABLED`/`WIKI_QA_ANSWER_LOG_FILE`（默认关闭，路径 `./logs/wiki-qa-answer.log`），按日记录脱敏的实际 prompt、模型文本、JSON校验状态及失败码，引用校验失败另记 failed；日志写入失败不影响问答。一次性 SSE/进程内运行记录；新增 `wiki_qa.progress` 事件，按 extract/retrieve/rerank/evidence/answer 发送 started/completed/failed/cancelled，带 actionRunId 与安全阶段文本；FE按阶段更新状态消息，重开还原进度，最终答案独立呈现。不创建 ACP 会话，不写 Ticket AI、Lark 或 wiki。 |
| `LarkTicketEvalSample` / `LarkTicketEvalReview` | Server workflow / PostgreSQL / FE projection | `server/src/application/services/lark-ticket-eval-dataset.service.ts`, `server/src/adapters/postgres/lark-ticket-eval-sample-store.ts` | 完整线程快照或意图、问题摘要、方案摘要齐全的不完整快照可创建冻结样本，直接纳入 Eval（正式输出优先，成功 Shadow 摘要兜底并冻结）；快照后续同步不覆盖历史样本；人工保存 Draft、Bad case、Eval 均由 Web Session 绑定审核人和服务端时间，样本与审核记录事务提交并按 actionRunId 幂等。旧数据不推断审核归属；Web 仅展示姓名、时间及本人参与标记。My evals 按历史参与在 Ticket 分页前去重过滤；两页复用紧凑只读状态标签及既有编辑入口。详见 [Eval 行为契约](../2026-09-01-lark-ticket-ai-output-and-eval-dataset-prd.md)。 |
| `LarkTicketShadowSummary` | Server background workflow / Ticket Summary adapter | `server/src/application/services/lark-ticket-shadow-summary.service.ts`, `server/src/domain/shadow-ticket-context.ts`, `server/src/domain/shadow-summary-prompt.ts` | 后台候选取得固定脱敏快照后，读取独立 `lark_ticket.shadow.summarize` 数据库 Prompt（缺失时使用内置模板），与 Quick Action 共用 provider/model。最终分析前复用 Wiki retrieve 完成问题提取、默认最多20篇候选召回（可配置）、重排及最多3篇原始证据筛选；空结果继续，失败明确标记 unavailable 并基于聊天分析。Wiki 使用 W 引用、thread 使用 M 引用，历史参考不得当作当前事实；保存私有 wikiEvidence 并仅透出 wikiContext 来源元信息。模型输入复用 Wiki 短标签、reply 关系及堆栈精简，保留所有沟通信号；60,000 字符消息预算按完整块保留最新、初始与可容纳的回复链，超长块整体省略并记录范围，原 preparedMessages 长 ID 不变。Server 生成完整性/同步时间，校验 `shadow-analysis-result-v2`、意图父子类型及实际发送消息的证据引用，去重后回映射原 ID。只写 `shadow_ai`，新增精简 businessRisk/replyAdvice（结论、rationale、evidenceMessageIds）、contextInfo、ruleVersion；风险 1–9/null 与 confidence 独立，回复五种且仅代表分析时建议。Web 白名单透出，FE 展示风险/回复建议、依据及材料局限；历史结果未评估。保留 processingDurationMs，安全 schema 诊断不记录原文。无 ACP、正式分析/Eval 写回或自动回复；不修改调度、推理参数、不自动重算。参见 [任务 v14](../../tasks/ai-ticket/2026-09-03-shadow-summary-worker.md)。 |
...
| `WebAiSessionRun` | Server application service / FE observes | `server/src/application/services/web-ai-session-runs.ts`, `fe/src/hooks/useAiSessionPanel.js` | Ticket/Sprint X 只收起显示；断连仅卸载观察者，后台任务继续。list/load 返回当前状态和事件快照，运行中不调用原生 load；显式 stop 校验身份、完整业务引用和精确 runId。Ticket 列表仅在页面可见且存在 running / waiting_permission / stopping 时每 10 秒刷新；首次进入、重新可见和会话操作主动刷新，空闲停止轮询。原生历史继续持久化，进程重启自动续跑不在本次范围。 |
| `LarkTicketAcpPermissionContext` | Server catalog / ACP proxy | `server/src/domain/acp-kimi-permission-profile.ts`, `server/src/application/services/acp-permission.service.ts` | Action/Profile 绑定会话及 effect draft。Kimi 回调继续校验 argv/path 并记录审计；Hermes 结构化 `patch`/`write_file` 只有在 diff、内容和既有 profile 路径策略全部通过时自动单次批准，其他原生风险请求按 options 等待当前 Web 用户回复并在 50 秒过期。FE Ticket/Sprint 共享审批卡片展示操作详情、diff 和倒计时，显式点击原生选项后继续或终止；待审批/失败不得被晚到完成事件标为验证成功。后台请求拒绝并取消，失败终态不能被 done 覆盖。能力声明不是 Hermes 原生 Terminal/`execute_code` 隔离保证。 |
...
| `server` | `ExtensionPageConfig`, `AutomationActionConfig`, `ResolvedUser`, `ExecutionDraft`, `WorkitemMapping`, `WorkflowPrompt`, `SupportTicketAnalysis`, `MeegleWorkitemRoleMember`, `MeegleWorkitemSprintMembership`, `MeegleSprintAiSession`, workflow request/result, `ActionRunTrace`, semantic field mapping, ACP one-shot limiter | action catalog、身份解析、授权检查、业务编排、Ticket 分析快照/证据校验、相关人清洗、Sprint 归属转换、workflow prompt、错误归一化、测试契约、可续聊 Sprint 会话控制 | 浏览器 DOM 细节、平台原始字段 shape、直接依赖 extension UI 状态 |
...
| `MeegleWorkitemLifecycleSnapshot` | `server` | PostgreSQL stores stable Sprint relation and scalar lifecycle projection, FE aggregates by day | current workitem detail + persisted Sprint snapshot -> canonical projection -> API -> Sprint chart | 从状态/节点/`updatedAt` 推断 start/finish、保留已被源端清空的旧值，或按可变 Sprint 名称关联 |
...
- Canonical lifecycle cleaning only reads the current workitem detail already fetched. `item_start_time` and `item_finish_time` map directly from `start_time` and `finish_time` to `YYYY-MM-DD`; missing, cleared, or invalid source values become null, and invalid nonempty values emit a structured warning. Do not infer either field from status, workflow nodes, creation time, old snapshot values, or `updatedAt`.
- Story System resolves `field_0dba3a` before fallback `field_00f541`; Tech Task uses `field_6da66b`; Production Bug uses `field_4976fc`. Only Odoo/Portal EU/US/UK normalize to `eu`/`us`/`uk`; empty or unrecognized current source clears the projection, with a structured warning for nonempty unrecognized values.
- Meegle incremental MQL `updated_at` is the source-version authority and is stored at second precision as `YYYY-MM-DD HH:mm:ss` UTC. Stores and FE must compare/parse this alongside legacy ISO values. An invalid observation is written and cleaned before the scope fails, so checkpoint CAS does not advance.
...
- Sprint history API projections must expand workitems from `MeegleWorkitemSprintMembership`, not regroup the current snapshot by its latest `sprint_id`. A closed original interval remains visible after A → B. Server may derive an outgoing “carryover to B” label only when the interval is `incremental_observed`, both Sprint date ranges are known, and A was unfinished at its end-day cutoff; FE renders that result without re-deriving history.
- Incoming membership classification is derived per request by a Server domain function over the ordered membership intervals plus Sprint date snapshots: `carryover` (prior different-Sprint interval that is `incremental_observed` with both Sprint date ranges known and unfinished at its end-day cutoff; carries the source Sprint id), `planned` (`added_at <= Sprint start + 24h`), `after_cycle`, and `unknown` (insufficient dates). Inferred intervals never derive historical Carryover — they classify as `unknown` — while inferred `planned`/`after_cycle` results are flagged `estimated`. FE renders these labels and the estimated flag without recomputing.
- `item_cycle_tag` and Carryover remain derived classifications and are not stored.
- Backfill of persisted intervals for existing workitems uses the idempotent `platform:init-sprint-memberships` entry (dry-run by default, `--apply` to write). It creates at most one open `historical_inferred` interval per workitem that currently has a Sprint (`added_at` from the stored `add_to_cycle_time`, else `max(workitem created time, Sprint start)`; start/finish from the PG-only projection clamped to `added_at`), skips workitems that already have an open interval, never requests Meegle, and cannot recover Sprint relations already overwritten in the snapshot. Before inserting, lock the current workitem snapshot row using the same lock order as incremental upsert, revalidate its Sprint, then check for an existing open interval.

...
| `SupportTicketAnalysis` | Server application service / PostgreSQL projection | `server/src/application/services/support-ticket-analysis.service.ts`, `server/src/adapters/postgres/support-ticket-analysis-store.ts`, `server/src/modules/lark-ticket/lark-ticket.controller.ts`, `server/src/modules/lark-ticket-ai/internal-lark-ticket-ai.controller.ts` | intent、result、quality 绑定固定 Ticket snapshot；Web PUT 负责人工更新，Summary ACP 经 SSH 签名 internal API 回写，两者共用校验和原子 upsert，证据 ID 必须来自 prepared messages |

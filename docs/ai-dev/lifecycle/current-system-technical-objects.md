...
| `SupportTicketAnalysis` | Server application service / PostgreSQL projection | `server/src/application/services/support-ticket-analysis.service.ts`, `server/src/adapters/postgres/support-ticket-analysis-store.ts`, `server/src/modules/lark-ticket/lark-ticket.controller.ts`, `server/src/modules/lark-ticket-ai/internal-lark-ticket-ai.controller.ts` | intent、result、quality 绑定固定 Ticket snapshot；Web PUT 负责人工更新，Summary Quick Action 通过受控 execute 拉取证据、写 workspace 分析文件并调用 `analysis-update`；Server 校验 snapshot/evidence 后更新规范化分析表与 FE `ticket_ai` 投影。证据或写回门禁失败时，Session 仍绑定 Ticket 并保存未验证正文与错误状态，但不得进入正式分析投影或直接发送。 |
...
| `LarkTicketAcpPermissionContext` | Server automation catalog / ACP proxy | `server/src/modules/public-config/automation-actions.config.ts`, `server/src/application/services/acp-kimi-permission-policy.ts`, `server/src/application/services/acp-kimi-execute-mcp-config.ts`, `server/src/adapters/kimi-acp/kimi-acp-runtime.ts` | Ticket 快捷动作将 Profile、Skill 和执行策略绑定至 Session；ACP fs 读取仅允许 Support workspace 与 Octo Server，写入仅允许 Support workspace；通用 Bash/terminal 默认拒绝，manifest 声明的脚本和 subcommand 通过结构化 `execute` MCP 逐次校验 |

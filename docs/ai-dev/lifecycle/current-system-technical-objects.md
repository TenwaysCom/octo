...
| `SupportTicketAnalysis` | Server application service / PostgreSQL projection | `server/src/application/services/support-ticket-analysis.service.ts`, `server/src/adapters/postgres/support-ticket-analysis-store.ts`, `server/src/modules/lark-ticket/lark-ticket.controller.ts` | intent、result、quality 绑定固定 Ticket snapshot；Web PUT 负责人工更新；Summary Quick Action 将脱敏固定快照交给 DeepSeek，Server 校验 JSON schema、snapshot version 与 evidence IDs 后直接更新规范化分析表及 FE `ticket_ai` 投影。失败时不得写入正式分析投影。 |
| `DeepSeekTicketSummaryAction` | Server workflow / DeepSeek adapter | `server/src/application/services/lark-ticket-ai-session.service.ts`, `server/src/adapters/deepseek/deepseek-chat-client.ts`, `server/src/domain/workflow-prompts.ts` | `lark-ticket-support-qa-summarize` 的一次性结构化分析；不启动 ACP、不创建可续聊 Session、不暴露 workspace 或工具权限。复用浏览器 SSE 外壳展示当前结果，长期结果由 `SupportTicketAnalysis` 投影承载。 |
| `LarkTicketShadowSummary` | Server background workflow / DeepSeek adapter | `server/src/application/services/lark-ticket-shadow-summary.service.ts`, `server/src/adapters/deepseek/deepseek-chat-client.ts`, `server/src/adapters/postgres/platform-sync-store.ts` | 后台候选取得固定脱敏快照后，读取 `lark_ticket.support_qa.summarize` 数据库 Prompt 并直连 DeepSeek；Server 校验 JSON schema 和 evidence IDs，只写独立 `shadow_ai` 投影。不会启动 ACP 或写正式 `SupportTicketAnalysis`。 |
...
| `LarkTicketAcpPermissionContext` | Server automation catalog / ACP proxy | `server/src/modules/public-config/automation-actions.config.ts`, `server/src/application/services/acp-kimi-permission-policy.ts`, `server/src/application/services/acp-kimi-execute-mcp-config.ts`, `server/src/adapters/kimi-acp/kimi-acp-runtime.ts` | Answer/Document Ticket 快捷动作将 Profile、Skill 和执行策略绑定至 Session；Summary 不使用该对象。ACP fs 读取仅允许 Support workspace 与 Octo Server，写入仅允许 Support workspace；通用 Bash/terminal 默认拒绝，manifest 声明的脚本和 subcommand 通过结构化 `execute` MCP 逐次校验。 |

...
| `LarkTicketAcpPermissionContext` | Server automation catalog / ACP proxy | `server/src/modules/public-config/automation-actions.config.ts`, `server/src/application/services/acp-kimi-permission-policy.ts`, `server/src/adapters/kimi-acp/kimi-acp-runtime.ts` | Ticket 快捷动作将 Profile、Skill 和四级执行策略绑定至 Session；每个 ACP 工具调用重新判定；Kimi 0.38 的完整参数按 `sessionId + toolCallId` 从先到达的 `tool_call` 单次关联，Support-QA fetch 未完成时 workflow 不接受结果 |

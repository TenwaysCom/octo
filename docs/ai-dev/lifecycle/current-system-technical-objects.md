...
| `TicketWikiQaQuickAction` / `WikiKnowledgeEvidence` | Server workflow / filesystem reader / completion clients | `server/src/application/services/wiki-qa.service.ts`, `server/src/adapters/filesystem/wiki-knowledge-reader.ts`, `server/src/adapters/ai/wiki-qa-rerank-client.ts` | `lark-ticket-wiki-qa` 获取固定 Ticket 聊天快照，提取问题、本地召回最多10篇 wiki 页面，仅前4组摘录/原始证据交给独立重排模型，选出最多3篇 FAQ / QA Card；index/entities 仅导航，raw 仅提供脱敏原始证据，Shadow AI 不作为原始证据。Server 分配引用编号，保留草稿与适用性边界，提取和回答仍使用共享问题总结模型；重排使用 `WIKI_QA_RERANK_MODE=general|rerank`：默认general为智谱 `glm-5.3-flash` Chat Completions；rerank为显式URL/model的专用query/documents协议，校验索引/分数后映射证据，保守标为历史参考/前提未核验。URL必须完整。现有超时不收紧；仅general默认地址可复用 `ZCODE_API_KEY`。提取阶段可通过 `WIKI_QA_EXTRACT_LOG_ENABLED=true` 开启专用按日日志（默认关闭），保存脱敏 prompt/输出及校验状态，以 actionRunId 关联；重排通过独立 `WIKI_QA_RERANK_LOG_ENABLED=true` 记录两种模式的脱敏请求/响应，默认关闭；不记录回答正文。一次性 SSE/进程内运行记录，不创建 ACP 会话，不写 Ticket AI、Lark 或 wiki。 |
| `LarkTicketEvalSample` / `LarkTicketEvalReview` | Server workflow / PostgreSQL / FE projection | `server/src/application/services/lark-ticket-eval-dataset.service.ts`, `server/src/adapters/postgres/lark-ticket-eval-sample-store.ts` | 完整线程快照创建冻结样本，直接纳入 Eval；人工保存 Draft、Bad case、Eval 均由 Web Session 绑定审核人和服务端时间，样本与审核记录事务提交并按 actionRunId 幂等。旧数据不推断审核归属；Web 仅展示姓名、时间及本人参与标记。My evals 按历史参与在 Ticket 分页前去重过滤；两页复用紧凑只读状态标签及既有编辑入口。详见 [Eval 行为契约](../2026-09-01-lark-ticket-ai-output-and-eval-dataset-prd.md)。 |

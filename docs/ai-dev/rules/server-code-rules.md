...
### Summary/Shadow thread context and Shadow result contract

Shadow uses its own `lark_ticket.shadow.summarize` prompt (`server/src/domain/shadow-summary-prompt.ts`), seeded only when absent; custom prompts and the formal Summary key remain untouched. The provider/model remain shared. Output is `shadow-analysis-result-v2`, prompt implementation version v6 and risk/reply rule version v1. Existing persisted v1 analyses remain readable; missing assessments mean unevaluated, not low risk.

`businessRisk` contains only `level` (integer 1–9 or null), `rationale`, and `evidenceMessageIds`; `replyAdvice` contains only `advice` (`reply_now`, `reply_by_deadline`, `normal_follow_up`, `no_reply_needed`, `undetermined`), `rationale`, and `evidenceMessageIds`. These are optional in the public historical projection but required on new model output. Intent/result confidence never calculates risk. The server also projects allowlisted `contextInfo` (source, historyComplete, dirty, syncedAt, checkedAt, totalMessages, includedMessages, truncated, omittedRanges, compactedMessages) and ruleVersion; metadata comes from the server, not the model.

Formal Summary and Shadow use the same `prepareTicketThreadAiContext` and `resolveTicketThreadEvidence` helpers. Model-only M aliases and external E reply references share Wiki formatting. Stored preparedMessages keep complete original IDs. Both Summary paths preserve acknowledgements, reminders and escalations, compact consecutive stack frames, and budget complete message blocks at 60,000 characters (Ticket fields/prompt/metadata are outside this message budget). Latest and initial blocks are selected first, followed by newest remaining blocks and their available reply ancestors, always rendered in snapshot order. Oversized blocks are omitted, not clipped; if latest and initial cannot both fit, latest wins. Omission ranges and snapshot freshness are explicit. Human-readable field forms are normalized and redacted, Issue Description falls back to detailDescription/title, and the user request does not duplicate it. Only presented message aliases may be cited; intent/risk/reply references are deduplicated and restored to original IDs before persistence. A referenced but omitted parent is not usable evidence. Incomplete materials must limit conclusions; no latest-message or real-time guarantee is implied.

Schema errors log safe allowlisted paths and missing/type/range codes with actionRunId, not values, dynamic score keys, raw parser exceptions or model text. New reply suggestions are advisory at analysis time. Candidate selection, retries, timeouts and effort remain unchanged; no prompt-version or chat-only reprocessing trigger is added.


Formal Summary retains its Ticket fields/user request, `lark_ticket.support_qa.summarize` database template and `support-analysis-result-v1` output. Server appends the short-reference contract to the rendered prompt, including custom/legacy templates: Message ID examples mean presented M labels, omitted/E references are invalid, and missing context limits claims. Formal intent evidence remains nonempty and unique; no complete message fitting the budget fails before provider invocation. Successful evidence is mapped to original IDs before the existing snapshot-version checked `analysisService.update`. No changes to ACP, Wiki selection, formal result fields, frontend, database content or automatic reprocessing are part of this thread unification.


Shadow now calls the existing Wiki `retrieve()` before final analysis: extract question → local recall → independently configured rerank → source evidence/relevance filtering (Top5 to at most3). It does not call Wiki `answer()`. Both Wiki QA and Shadow use `WIKI_QA_RECALL_LIMIT` (default20, integer1..100); invalid values fail retrieval rather than silently falling back. Dedicated rerank thresholds and general-mode semantics are unchanged. Extraction uses existing Wiki effort/provider settings; final Shadow generation keeps its settings. Total processingDurationMs now includes retrieval; a populated run normally adds one extraction and one rerank request, without new worker parallelism/retries/timeouts.

Retrieval failures degrade to chat-only analysis as requested. Server appends Wiki materials and evidence-boundary instructions even to custom Shadow prompts. `wikiContext.status` is `matched`, `no_matches` or `unavailable`; safe errorCode and selected source metadata (sourceId/title/path/status/applicability/limitations) are persisted and projected. Selected redacted excerpts are stored as private `wikiEvidence`, not exposed in the Web projection. No sources means no valid W references; final `[W1]`-style markers are validated against this run's sources. `evidenceMessageIds` remains exclusively current-thread M references. Historical Wiki facts must not become current Ticket resolution, risk, response or quality facts. Existing model output objects remain unchanged. FE shows retrieval status, references and limitations; Lark App displays unavailable status without expanding details. Historical results without wikiContext remain readable. No automatic reprocessing or formal Summary Wiki integration is added.

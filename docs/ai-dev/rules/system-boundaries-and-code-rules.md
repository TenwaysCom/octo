...
Lark Bug 分析 workflow 还应遵守：

1. Extension 只触发 server-owned backend action and displays returned status; 是否在插件内展示完整分析正文是单独 UI contract，不能默认为 backend action 的职责。
2. Server workflow reads the Meegle Production Bug workitem or Lark record, then runs ACP analysis.
3. Meegle workitem input overwrites `analysisSummary`; Lark record input appends analysis to Lark Base `Details Description`.
4. ACP execution is one-shot: create runtime, run one prompt, close runtime.
5. One-shot ACP sessions are not reusable chat sessions and must not be written to reusable session ownership state.
6. Concurrency and timeout failures return typed server responses and must happen before any platform writeback.
7. Prompt template lives in PostgreSQL `workflow_prompts` under key `lark.bug.analyze`, with `note` describing the prompt owner/usage.
8. Prompt variables must use the shared double-brace format, for example `{{bug_description}}`; single-brace `{bug_description}` is not rendered and should be treated as a prompt configuration bug.
9. Lark record input must use a real Base `recordId` (`rec...`). `/record/{wikiRecordId}` is context only and must not be passed to Bitable `getRecord`/`updateRecord`.
10. On `/record/{wikiRecordId}` pages, the extension should read the real record id from the current page's `记录ID` field before calling the backend; URL path `wikiRecordId` is only kept as diagnostic/context data.
11. Prompt variable `bug_description` should be populated from the `Lark Message Link` target message when available, then fall back to record fields.

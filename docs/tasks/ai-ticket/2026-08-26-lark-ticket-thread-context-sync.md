...
- 历史补齐使用 `platform:backfill-finished-lark-ticket-threads`：仅选择 `ticket_status=Finish`、存在可解析 thread link、且没有同一 thread 的完整快照的记录；必须显式传入 `baseId + tableId`，避免跨表批量调用。
...
| v2 批量补齐脚本单测 | 通过 | `pnpm --dir server exec vitest run src/scripts/backfill-finished-lark-ticket-threads.test.ts src/application/services/lark-ticket-thread-context.service.test.ts`：2 files / 10 tests | 覆盖参数保护、候选筛选、限并发与复用的 thread ensure 决策；未调用 Lark。 |

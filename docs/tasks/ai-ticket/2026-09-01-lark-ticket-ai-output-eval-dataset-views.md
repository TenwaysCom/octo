...
按用户确认的简洁 UI 实现 Eval 状态、人工保存归属和 My evals。保留现有布局：标题前单个只读标签显示“未标记 / Draft / Bad case / 纳入 Eval”；右侧继续使用 Thread、messages、“加入 Eval / 编辑 Eval”入口。标签不提供菜单。

## 验收标准（v4）

- [x] 两页标题前统一紧凑状态标签，无新状态栏、统计面板或常驻人/时间列。
- [x] “加入 Eval”直接纳入并记录操作者/时间；已有样本显示“编辑 Eval”，复用现有弹窗（Eval 页保留铅笔入口）。
- [x] Draft 和 Eval 标注选填；Bad case 至少选择一个失败标签，人工意图和期望结果选填；前后端一致。
- [x] 人工成功保存任意状态均记录 Web Session 操作人及服务端时间；失败保存、取消编辑不产生审核记录。
- [x] 状态悬浮信息及现有编辑器显示最近 Eval 人/时间，历史缺失显示未记录，不猜测补齐。
...
- v4 是当前实施口径，替代 v2/v3 的待定入口和展示方案。用户已要求按最终讨论实现。
- “未标记”表示尚无样本，数据库状态仍为 draft / badcase / eval。直接加入后可通过编辑器保存为 Draft；不增加额外按钮。
...
| 前置 / 操作 | 预期 |
| --- | --- |
| 完整线程快照且未建样本，AI 输出行点击“加入 Eval” | 标题前“未标记”变为“纳入 Eval”，右侧变为“编辑 Eval”；无需填写标注 |

...
新增：快照不完整但意图、问题摘要、方案摘要齐全时允许加入 Eval；创建失败使用右上角红色通知，5 秒后自动关闭。快照仍由现有同步流程后续补完，本次不新增补齐入口或改写历史样本。

## 验收标准（v6）

- [x] 不完整快照在三项摘要齐全时可加入 Eval（正式输出优先，成功 Shadow 输出兜底），冻结有效输出；缺少快照或摘要不足仍拒绝。
- [x] 创建失败显示右上角红色通知，5 秒关闭，可手动关闭；重复错误重新计时。
- [x] 定向单测、FE 检查及 Server 构建通过，记录未部署边界。
...
- v6 替代历史“快照不完整一律拒绝”的限制；完整快照仍允许 AI 输出为空。样本继续绑定创建时快照版本，后续同步不覆盖历史样本。复用列表通知，Eval 创建错误单独指定 5000ms。
...
| 类型 | 结果 | 边界 |
| --- | --- | --- |
| Server 定向单测 | Eval service 12 tests 通过 | 覆盖完整快照、正式/Shadow/混合摘要准入、正式优先、缺项或缺快照拒绝、原有幂等；使用 mock store |
| FE 自动化 | `pnpm --dir fe check` 通过 | 现有测试与 Vite 构建；通知位置、红色样式和 5000ms 清理逻辑经代码检查，未做浏览器计时验收 |
| 静态检查 | `pnpm --dir server build`、`git diff --check` 通过 | 未改数据库结构，无新增迁移 |
| Live E2E / 已部署运行时 | 未执行 | 需发布 FE、部署/重启 Server 后验收 |

进展：v6 已实现三项摘要齐全的不完整快照加入 Eval，冻结准入使用的 Shadow 摘要；失败复用列表右上角通知并指定 5 秒自动关闭。原有工作区 Wiki QA 改动保留。快照补齐沿用既有同步机制，历史 Eval 样本不自动换绑到新快照。

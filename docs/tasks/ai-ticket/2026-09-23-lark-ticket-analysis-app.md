...
v35：公共区展示 Ticket 客观信息和带来源的 AI 诉求理解；同步时间置于属性栏，Shadow 时间置于其标题旁，诉求提示改为标题 i 图标 tooltip；默认已有分析页签；即时分析页签手动接入已有 Wiki 问答及当前用户历史，已有分析页签仅展示 Shadow AI，默认预览意图、关键词、问题总结、方案摘要，意图和方案摘要旁显示带悬停/聚焦 tooltip 的浅品牌色 AI 自评徽标；公共意图也展示同来源自评分，其余字段折叠，回复草稿置于 Shadow AI 页签顶部；公共诉求与回复参考仍保留原有来源规则。风险、回复时机、最佳动作保留规划。仅 FE 改动，无新接口或迁移。
...
- [x] v35 统一 FE 基础色、语义色和组件色变量；Lark App 使用统一中性色及青绿信息色，琥珀主色保持不变。

- [x] v34 Lark App 使用琥珀主色/青绿副色：主操作和选中态保留品牌色，字段/信息图标/信息徽标使用青绿与浅青绿。

- [x] v33 优化公共诉求卡片层级：紧凑意图行、突出摘要阅读区；保留来源、完整内容及 tooltip。

- [x] v32 公共诉求来源一致时，来源 badge 放在 AI 诉求理解标题后，字段不重复标注；混合来源仍逐项标注。
...
- 按用户截图要求，公共意图/诉求来源一致时将 Shadow AI（或实际共同来源）的浅色 badge 放在“AI 诉求理解”标题后、i 图标前，字段标签不再重复来源。混合来源仍逐项标注，避免把 Ticket AI 内容误标成 Shadow；没有来源时不显示 badge。
- 验证：FE 构建与 `git diff --check` 通过；仅展示调整，未新增/重跑单测或浏览器 mock。未提交、未部署。保留其他任务的 Server 与 Shadow worker 文档变更。

## v33 公共诉求卡片层级优化

- 标题后保留共同来源 badge，i 说明图标靠右；意图标签/分类与自评分采用紧凑行，长内容自然换行。摘要使用独立分隔线、14px 字号及 1.8 行高突出阅读，保留完整文本与缺失态，不增加常驻提示。仅调整公共诉求 JSX/className 与局部 CSS，未修改同期其他任务新增的 Shadow 风险展示。
- 验证：FE 构建与 `git diff --check` 通过；360px/768px mock 使用合成长摘要和英文意图分类验证无横向溢出、来源去重及 tooltip/时间/用户组件回归，均通过；已目检 `/tmp/lark-app-v33-360.png`、`/tmp/lark-app-v33-768.png`。临时脚本 `/tmp/lark-app-v33-check.mjs`。未重跑单测、未提交或部署。约 507 kB chunk 构建提示非阻断。

## 回复草稿提示精简

- 按用户要求删除“复制后自行粘贴到 thread”提示行，编辑与复制行为保持不变。源码核对与 `git diff --check` 通过；纯文案删除，未重跑构建/测试，未提交或部署。

## 字段名称颜色

- 用户明确 label 指“意图推断、诉求摘要”等字段名称。将 Lark App 的 dt 与输入框 label 改为蓝灰色 `#526d92`、600 字重，区分正文与浅品牌色徽标；来源和 AI 自评分 badge 配色不变。
- 验证：样式值与 `git diff --check` 通过；纯颜色/字重调整，未重跑构建/测试，未提交或部署。

## 诉求摘要同行展示

- 按用户要求将“诉求摘要”标签与正文改为同一行起始，正文在右侧列自然换行，保留摘要分隔线及完整内容。
- 验证：CSS 与 `git diff --check` 核对通过；纯布局样式变动，未重跑构建/测试，未提交或部署。

## 回复草稿重复标签精简

- 按用户截图要求，隐藏输入框上方重复的“回复草稿”标签，保留卡片标题与复制按钮；使用现有 visually-hidden 样式保留输入框可访问名称。
- 验证：标签关联/样式与 `git diff --check` 核对通过；纯展示调整，未重跑构建/测试，未提交或部署。

## 字段名称使用主题色

- 用户要求利用现有主题 schema 的颜色。字段名称改用 `--octo-badge-info-text`（当前 #0369a1），替换上一轮硬编码蓝灰色；维持略加粗、正文色和品牌徽标色，不额外新增颜色 token。
- 验证：主题变量存在、两处字段样式引用及 `git diff --check` 核对通过；纯颜色替换，未重跑构建/测试，未提交或部署。

## v34 琥珀与青绿配色

- 用户确认采用双配色。主按钮、选中页签和 Logo 保持琥珀品牌色 #B45309；字段名称、信息图标、来源及 AI 自评徽标使用主题青绿 #0F766E，信息浅底使用 #F0FDFA。人工整理保留浅品牌色，业务状态颜色不变；接入说明同步更新。
- 验证：FE 构建通过；360px/768px 浏览器 mock 验证青绿文字与浅底、琥珀主色保持不变、长文本无横向溢出及 tooltip 回归通过，目检 360px 截图。脚本 `/tmp/lark-app-v34-check.mjs`，截图 `/tmp/lark-app-v34-360.png` 与 `/tmp/lark-app-v34-768.png`。纯配色调整，未重跑全量单测，未提交或部署，未做真实 Lark 验证。约 507 kB chunk 构建提示仍非阻断；保留其他任务修改。

## v35 统一颜色语义

- 将 FE 公共主题整理为基础色、语义色和组件色三层：琥珀主色不变，青绿信息色，统一 slate 正文/辅助文字/背景/边框，警告改为金黄色。保留已有变量兼容映射；公共变量影响主站引用区域，其他页面硬编码未逐项迁移。
- Lark App 移除局部硬编码颜色，字段使用 field-label 组件变量，来源 badge 使用 source-badge 变量，自评/i 图标使用 info 语义变量；人工整理使用浅品牌色，草稿输入区保持白色。评分不映射状态色。更新 FE README 与接入说明。
- 验证：FE 构建通过（现有约 507 kB chunk 提示非阻断），复用 `/tmp/lark-app-v34-check.mjs` 的 360px/768px mock 验证通过，包括配色、主色不变、长文布局、tooltip 与静态用户信息；`git diff --check` 通过。纯样式调整未重跑全量单测，主站未逐页目检；未做真实 Lark 验证，未提交或部署。保留工作区其他任务修改。

## v35 提交检查

- 用户授权提交本轮公共卡片、回复草稿精简及统一配色变更。公共卡片文件中的风险/回复时机预览属于并行 Shadow worker 任务，不纳入本次提交；其他任务已暂存文档及工作区修改保留。
- 提交前工作区 FE check 通过（46 个测试文件与构建），配色/窄屏 mock 证据见 v35。未部署、未推送；现有 bundle 大小提示非阻断。

...
- [x] Sprint 工作项表格默认显示 Related PR 列，支持多条关联 PR 链接和状态展示。
...
v1 的历史列表筛选/分组决策已 superseded。v2 的列/排序视图配置由 v3 扩展为 Group by、Sub group by、列和排序；v4 在两级分组选项中加入 Version；v5 将 Version 同时加入默认展示列与排序配置；v6 复用 Server 已返回的 `githubPullRequests`，在工作项表格展示 Related PR。筛选控制类型、状态、项目、优先级和负责人。二者均在当前 Sprint 的已加载 work item 集合上执行。配置归一化、排序与分组保持在独立纯函数中，便于单测。
...
| 2026-08-28 | v6 | in_progress | Sprint 工作项表格增加默认可见的 Related PR 列，复用已同步的关联 PR 摘要。 | 待完成 FE 验证。 |
| 2026-08-28 | v6 | done | Related PR 默认显示并支持列配置；多条关联逐条展示 PR 编号、目标分支、状态和外链，无关联时显示 `-`。 | 未执行已登录浏览器视觉验收。 |
...
| FE 单测 / 构建（v6） | 通过 | `pnpm --dir fe check`：23/23 test files passed；Vite production build passed。新增 Related PR 默认列配置覆盖。 | 未执行已登录浏览器视觉验收。 |

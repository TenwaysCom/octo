...
- `Meegle User Story / Meegle Product Bug` 在适配层里应映射为 `Meegle workitem`
...
> `Meegle User Story / Meegle Product Bug` 是 Meegle 顶层执行工作项；更细粒度的执行动作可能落在 workflow / task 层，而不是顶层 workitem。

...
export function LarkTicketAiWorkspace({ apiBaseUrl, myEvals = false, mode, groups, visibleColumns, collapsedGroups = [], onTicketContextMenu, onLarkTicketDetailLinkClick, onToggleGroup = () => {}, collapsedSubgroups = [], onToggleSubgroup = () => {} }) {
  ...
  async function createSample(ticket) { setCreatingId(ticket.recordId); setError(""); try { const sample = await createLarkTicketEvalSample({ apiBaseUrl, ticket, actionRunId: crypto.randomUUID() }); setSamples((current) => [sample, ...current.filter((item) => item.id !== sample.id)]); } catch (cause) { const code = cause?.code || cause?.message; setError(code === "THREAD_SNAPSHOT_INCOMPLETE" ? "该 Ticket 的线程快照不完整，不能创建 Eval 样本。" : code === "THREAD_SNAPSHOT_NOT_FOUND" ? "该 Ticket 尚未生成线程快照，请先进入详情页完成 AI 上下文准备。" : `创建 Eval 样本失败：${code || "UNKNOWN_ERROR"}`); } finally { setCreatingId(null); } }

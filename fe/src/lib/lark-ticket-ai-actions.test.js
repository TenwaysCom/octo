...
test("Problem Summary and wiki QA are one-shot actions", () => {
  assert.deepEqual(
    LARK_TICKET_AI_QUICK_ACTIONS.filter((action) => action.oneShot).map((action) => action.actionKey),
    ["lark-ticket-support-qa-summarize", "lark-ticket-wiki-qa"],
  );
  assert.equal(isOneShotLarkTicketAiAction("lark-ticket-support-qa-summarize"), true);
  assert.equal(isOneShotLarkTicketAiAction("lark-ticket-wiki-qa"), true);

...
import { replyAcpPermission, listMeegleSprintAiSessions, loadMeegleSprintAiSession, stopMeegleSprintAiSession, streamMeegleSprintAiSession } from "../services/meegle-sprint-ai/meegle-sprint-ai-api.js";
...
function SprintAiSessions({ sprint, apiBaseUrl }) {
  ...
  const { drawer, panel, isStreaming } = useAiSessionPanel(JSON.stringify([apiBaseUrl, sprint.projectKey, sprint.sprintId]), {
    load: (sessionId) => loadMeegleSprintAiSession({ apiBaseUrl, sprint, sessionId }),
    stream: (input) => streamMeegleSprintAiSession({ ...input, apiBaseUrl, sprint }),
    stop: (sessionId, runId) => stopMeegleSprintAiSession({ apiBaseUrl, sprint, sessionId, runId }),
    onChange: refresh,
  });
  ...
  useEffect(() => {
    ...
    const refreshList = async () => {
      ...
      if (active) timer = setTimeout(refreshList, 3000);
    };
    void refreshList();
  ...
  async function openSession(session) {
    setDrawerDraft("");
    await panel.open(session);
  }

  async function streamSession(input) {
    await panel.start(input);
  }
  ...
  return <section className="sprint-ai-sessions" aria-label="Sprint AI Sessions">
    ...
    {aiSessions.items.length ? <div className="ticket-ai-session-list">{aiSessions.items.map((session) => <button className="ticket-ai-session-card" type="button" key={session.sessionId} onClick={() => void openSession(session)}><span className="ticket-ai-session-card__icon" aria-hidden="true">✦</span><span className="ticket-ai-session-card__content"><strong>{session.title}</strong><small>{formatDateTime(session.updatedAt)}{session.runStatus ? ` · ${aiRunStatusLabel(session.runStatus)}` : ""}</small></span><span className="ticket-ai-session-card__open" aria-hidden="true">›</span></button>)}</div> : null}
    ...
    {drawer ? <div className="ticket-ai-drawer-backdrop" role="presentation" onMouseDown={() => panel.close()}><aside className="ticket-ai-drawer" aria-label="Sprint AI Session 详情" onMouseDown={(event) => event.stopPropagation()}><header className="ticket-ai-drawer__header"><div><p>Sprint AI Chat</p><h2>{drawer.title}</h2></div><button type="button" aria-label="关闭 AI Session" onClick={() => panel.close()}>×</button></header><div className="ticket-ai-drawer__body">{drawer.status === "loading" ? <p className="ticket-section-empty">正在加载会话…</p> : null}{drawer.messages.map((entry, index) => <div className={`ticket-ai-message ticket-ai-message--${entry.kind}`} key={entry.id || `${entry.kind}-${index}`}>{entry.kind === "permission" ? <AcpPermissionPrompt permission={entry.permission} active={isStreaming} onReply={(input) => replyAcpPermission({ ...input, apiBaseUrl })} /> : null}{entry.text ? <div className="ticket-ai-message__text">{entry.text}</div> : null}{entry.kind === "assistant" && entry.text ? <div className="ticket-ai-message__actions"><AiSessionCopyButton text={entry.text} /></div> : null}</div>)}{drawer.connectionError ? <p role="status">{drawer.connectionError}</p> : null}{drawer.status === "cancelled" ? <p role="status">已停止生成，已有内容已保留。</p> : null}{drawer.status === "stopping" ? <p role="status">正在停止生成…</p> : null}{drawer.status === "generating" ? <p className="ticket-ai-generating">AI 正在生成回复…</p> : null}{drawer.status === "error" ? <div className="ticket-ai-drawer__error"><p>{drawer.error}</p>{drawer.lastMessage ? <button type="button" disabled={isStreaming} onClick={() => void streamSession({ message: drawer.lastMessage, sessionId: drawer.sessionId || undefined, title: drawer.title })}>Retry</button> : null}</div> : null}</div><form className="ticket-ai-drawer__composer" onSubmit={(event) => { event.preventDefault(); const message = drawerDraft; setDrawerDraft(""); void streamSession({ message, sessionId: drawer.sessionId || undefined, title: drawer.title }); }}><label className="visually-hidden" htmlFor="sprint-ai-followup">继续对话</label><textarea id="sprint-ai-followup" value={drawerDraft} onChange={(event) => setDrawerDraft(event.target.value)} placeholder="继续这个 AI Session…" rows="2" disabled={isStreaming || drawer.status === "loading"} /><div className="ticket-ai-drawer__composer-actions">
      ...
      <button className="ticket-ai-drawer__send" type="submit" aria-label="发送" title="发送" disabled={isStreaming || drawer.status === "loading" || !drawerDraft.trim()}>
...
function SprintWorkitemGroupedList({ groups, groupKeyPrefix, collapsedGroups, collapsedSubgroups, onToggleGroup, onToggleSubgroup, sort, visibleColumns, onSort, apiBaseUrl, nowTime }) {
  return <div className="grouped-list">{groups.map((group) => {
    const groupKey = `${groupKeyPrefix}::${group.key}`;
    const collapsed = collapsedGroups.includes(groupKey);
    return <section className="grouped-list__section" key={groupKey}>
      <button className="grouped-list__header" type="button" aria-expanded={!collapsed} onClick={() => onToggleGroup(groupKey)}>
      ...
      {!collapsed ? group.subgroups?.length ? <div className="grouped-list__subgroups">{group.subgroups.map((subgroup) => {
        const subgroupKey = `${groupKeyPrefix}::${subgroup.key}`;
        const subgroupCollapsed = collapsedSubgroups.includes(subgroupKey);
        return <section className="grouped-list__subgroup" key={subgroupKey}>
          <button className="grouped-list__subgroup-header" type="button" aria-expanded={!subgroupCollapsed} onClick={() => onToggleSubgroup(subgroupKey)}>
          ...
          {!subgroupCollapsed ? <SprintWorkitemList apiBaseUrl={apiBaseUrl} items={subgroup.items} nowTime={nowTime} sort={sort} visibleColumns={visibleColumns} onSort={onSort} /> : null}
        </section>;
      })}</div> : <SprintWorkitemList apiBaseUrl={apiBaseUrl} items={group.items} nowTime={nowTime} sort={sort} visibleColumns={visibleColumns} onSort={onSort} /> : null}
    </section>;
  })}</div>;
}

function SprintWorkitemSections({ sections, groupBy, collapsedGroups, collapsedSubgroups, onToggleGroup, onToggleSubgroup, sort, visibleColumns, onSort, apiBaseUrl, nowTime }) {
  return <div className="sprint-workitem-sections">{sections.map((section) => <section className="sprint-workitem-section" key={section.key}>
    <header className="sprint-workitem-section__header"><strong>{section.label}</strong><span>{section.items.length} 条</span></header>
    {groupBy === "none"
      ? <SprintWorkitemList apiBaseUrl={apiBaseUrl} items={section.items} nowTime={nowTime} sort={sort} visibleColumns={visibleColumns} onSort={onSort} />
      : <SprintWorkitemGroupedList apiBaseUrl={apiBaseUrl} groups={section.groups} groupKeyPrefix={section.key} collapsedGroups={collapsedGroups} collapsedSubgroups={collapsedSubgroups} onToggleGroup={onToggleGroup} onToggleSubgroup={onToggleSubgroup} nowTime={nowTime} sort={sort} visibleColumns={visibleColumns} onSort={onSort} />}
  </section>)}</div>;
}
  ...
  return <div className="list-view-config-panel sprint-workitem-view-config-panel">
    ...
    <div className="list-view-config-section list-view-config-section--ordering">
      ...
      <select id="meegle-sprint-workitem-order-by" value={sort.key} onChange={(event) => onSortChange({ ...sort, key: event.target.value })}>
        {SPRINT_WORKITEM_VIEW_COLUMNS.filter(({ sortKey }) => sortKey).map(({ label, sortKey }) => <option key={sortKey} value={sortKey}>{label}</option>)}
      </select>
...
export function MeegleSprintDetailPage({ profile, sprintName, apiBaseUrl, onLogout, isBusy, breadcrumbs, meegleSprintDetailState, onMeegleSprintDetailStateChange }) {
  const initialPageState = normalizeSprintWorkitemPageState(meegleSprintDetailState);
  const state = useMeegleSprintHistory(apiBaseUrl);
  const [panelOpen, setPanelOpen] = useState(true);
  const [filterOpen, setFilterOpen] = useState(false);
  const [viewConfigOpen, setViewConfigOpen] = useState(false);
  const [activeFilterFieldKey, setActiveFilterFieldKey] = useState("status");
  const [selectedWorkitemFilters, setSelectedWorkitemFilters] = useState(() => initialPageState.selectedWorkitemFilters);
  const [groupBy, setGroupBy] = useState(() => initialPageState.groupBy);
  const [subGroupBy, setSubGroupBy] = useState(() => initialPageState.subGroupBy);
  const [sort, setSort] = useState(() => initialPageState.sort);
  const [visibleColumns, setVisibleColumns] = useState(() => initialPageState.visibleColumns);
  const [collapsedGroups, setCollapsedGroups] = useState([]);
  const [collapsedSubgroups, setCollapsedSubgroups] = useState([]);
  const workingTimeNow = useMinuteNow(visibleColumns.includes("currentWorkingTime"));
  ...
  const workitemSections = splitSprintWorkitemsByCarryover(sortedItems).map((section) => ({
    ...section,
    groups: groupSprintWorkitems(section.items, groupBy, { subGroupBy }),
  }));
  pageStateRef.current = { selectedWorkitemFilters, groupBy, subGroupBy, sort, visibleColumns };
  useEffect(() => () => {
    if (pageStateRef.current) onMeegleSprintDetailStateChange?.(sprintName, pageStateRef.current);
  }, [onMeegleSprintDetailStateChange, sprintName]);
  useEffect(() => {
    if (groupBy === "none") {
    ...
    setCollapsedGroups([...new Set(workitemSections.flatMap((section) => section.groups.map((group) => `${section.key}::${group.key}`)))]);
    setCollapsedSubgroups([...new Set(workitemSections.flatMap((section) => section.groups.flatMap((group) => (group.subgroups || []).map((subgroup) => `${section.key}::${subgroup.key}`))))]);
  }, [groupBy, subGroupBy, workitemSections]);
  useKeyboardShortcut({
    key: "Escape",
      ...
      {state.status === "ready" && sprint ? <>
        ...
        <div className={`sprint-detail-layout ${panelOpen ? "sprint-detail-layout--with-panel" : ""}`.trim()}>
          <div className="sprint-detail-layout__main">
            {sortedItems.length ? <SprintWorkitemSections apiBaseUrl={apiBaseUrl} sections={workitemSections} groupBy={groupBy} collapsedGroups={collapsedGroups} collapsedSubgroups={collapsedSubgroups} onToggleGroup={(key) => setCollapsedGroups((current) => current.includes(key) ? current.filter((item) => item !== key) : [...current, key])} onToggleSubgroup={(key) => setCollapsedSubgroups((current) => current.includes(key) ? current.filter((item) => item !== key) : [...current, key])} nowTime={workingTimeNow} sort={sort} visibleColumns={visibleColumns} onSort={(key) => updateSort({ key, direction: sort.key === key && sort.direction === "asc" ? "desc" : "asc" })} /> : <p className="list-message">当前筛选条件下没有工作项。</p>}
          </div>

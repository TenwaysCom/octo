...
function SyncSourceCard({ source, latest, running, error, onSync }) {
  ...
  return <article className="sync-source-card">
    <div className="sync-source-card__heading">
      ...
      <span className={`status-badge ${configured && ready ? "status-badge--ready" : "status-badge--attention"}`}>
        <span />{!configured ? "未配置" : ready ? "已同步" : "待同步"}
      </span>
    </div>
    <dl className="authorization-details">
      <div><dt>最近同步</dt><dd>{latest ? formatDateTime(latest) : "暂无同步记录"}</dd></div>
      <div><dt>状态</dt><dd>{!configured ? "服务端未配置" : ready ? "快照可用" : "等待首次同步"}</dd></div>
    </dl>
    {error ? <p className="profile-card__error">{error}</p> : null}
    <button className="secondary-button" type="button" disabled={!configured || running} onClick={onSync}>
      {running ? "同步中…" : "立即同步"}
    </button>
  </article>;
}

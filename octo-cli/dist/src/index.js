...
import { getActiveProfile, getConfigPath, listProfiles, loadConfig, redactConfig, removeProfile, saveConfig, setActiveProfile, setProfileStrictMode, } from "./config.js";
...
export async function run(argv, stdout = console.log) {
    ...
    if (command === "profile")
        return runProfile(subcommand, operation, flags, profile, stdout);
    if (command === "schema")
        return runSchema(subcommand, stdout);
    if (command === "doctor")
        return runDoctorCommand(profile, flags, stdout);
    if (command === "upgrade")
        return runUpgrade(flags, stdout);
    if (command === "skills")
...
async function runProfile(subcommand, operation, flags, profile, stdout) {
    if (subcommand === "list") {
    ...
    if (subcommand === "strict-mode") {
        ...
        if (operation !== "on" && operation !== "off")
            throw new Error("strict-mode must be on or off.");
...
async function runUpgrade(flags, stdout) {

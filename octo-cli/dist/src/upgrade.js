...
export async function checkForUpgrade(manifestUrl, dependencies = {}) {
    ...
    if (!response.ok)
        throw new Error(`UPGRADE_MANIFEST_REQUEST_FAILED: ${response.status} ${response.statusText}`.trim());
...
export async function applyUpgrade(check, dependencies = {}) {
    if (!check.updateAvailable)
        return { updated: false, version: check.currentVersion };
    const fetchImpl = dependencies.fetchImpl ?? fetch;
    ...
    if (!response.ok)
        throw new Error(`UPGRADE_ARTIFACT_REQUEST_FAILED: ${response.status} ${response.statusText}`.trim());
...
export function resolveManifestUrl(flagValue, environment = process.env) {
...
export function resolveUpdateToken(environment = process.env) {
...
function parseVersion(version) {
    if (!isReleaseVersion(version))
        throw new Error(`UPGRADE_VERSION_INVALID: ${version} is not a stable semantic version.`);
...
function requestHeaders(base, updateToken) {
    const headers = new Headers(base);
    if (updateToken)
        headers.set("authorization", `Bearer ${updateToken}`);
    return headers;
}

...
test("serves all five CLI demo data projections behind a bearer token", async () => {
    ...
    try {
        ...
        const burndown = await api.get(`/api/agent/v1/projects/${DEMO_PROJECT_KEY}/sprints/${DEMO_SPRINT_ID}/burndown`);
        const tasks = await api.get(`/api/agent/v1/projects/${DEMO_PROJECT_KEY}/sprints/${DEMO_SPRINT_ID}/tasks`);
        const pr = await api.get("/api/agent/v1/github/pull-requests/TenwaysCom/octo/42");
        const ticket = await api.get("/api/agent/v1/lark-tickets/base-demo/table-tickets/rec-demo-1001");
        const odooBranches = await api.get("/api/agent/v1/odoo/branches", { environment: "eu" });

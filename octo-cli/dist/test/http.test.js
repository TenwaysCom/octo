...
test("sends an agent token and unwraps successful data", async () => {
    let requestUrl;
    let authorization;
    const client = new OctoApiClient({ serverUrl: "https://octo.example/", apiToken: "agent-token" }, { fetchImpl: async (input, init) => {
...
test("returns the Octo error code for failed requests", async () => {
    const client = new OctoApiClient({ serverUrl: "https://octo.example", apiToken: "agent-token" }, { fetchImpl: async () => new Response(JSON.stringify({ ok: false, error: { errorCode: "SNAPSHOT_NOT_FOUND" } }), { status: 404 }) });

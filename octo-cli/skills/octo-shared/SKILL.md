...
4. Use the selected Profile consistently. Do not switch, remove, overwrite, or change its strict mode unless the user explicitly asks.
...
Use `octo-cli profile strict-mode on --name <profile>` for a production Profile. Strict mode binds requests to its configured server URL and rejects a conflicting `OCTO_SERVER_URL`. Server URLs must use HTTPS except for local demo loopback addresses.

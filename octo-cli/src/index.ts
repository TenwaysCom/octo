...
export async function run(argv: readonly string[], stdout: (value: string) => void = console.log): Promise<void> {
...
async function printApiResult(result: Promise<unknown>, profile: string, stdout: (value: string) => void): Promise<void> {
...
function runSchema(name: string | undefined, stdout: (value: string) => void): void {
  if (name) {
...
async function runAgentInstall(flags: ReadonlyMap<string, string>, stdout: (value: string) => void): Promise<void> {
  const agent = flags.get("agent") || "codex";
  if (agent !== "codex") throw new Error("Only the Codex skill target is supported in v0.1.");
...
async function runDemoServer(flags: ReadonlyMap<string, string>, stdout: (value: string) => void): Promise<void> {
...
function printSuccess(data: unknown, stdout: (value: string) => void, meta?: { profile?: string; count?: number }): void {

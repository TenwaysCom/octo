...
test("does not propose an equal or lower release", async () => {
  const fetchImpl: typeof fetch = async () => new Response(JSON.stringify(release("0.1.0")), { status: 200 });
...
test("verifies the downloaded artifact before invoking npm", async () => {
  ...
  const result = await applyUpgrade(check, {
    fetchImpl: async (input) => {
      if (String(input).endsWith("latest.json")) return new Response(JSON.stringify(release()), { status: 200 });

...
import type { AcpSpawnConfig } from "./spawn-config.js";
...
export interface AcpConnectionFactoryInput {
...
export interface AcpRuntimeDeps {
  ...
  buildSpawnConfig: (env: NodeJS.ProcessEnv) => AcpSpawnConfig;
  spawnProcess?: typeof spawn;
  createConnection?: (
    input: AcpConnectionFactoryInput,
  ) => Promise<AcpConnection> | AcpConnection;
  emit?: (event: AcpKimiStreamEvent) => void;
  capabilityPolicy?: AcpKimiClientCapabilityPolicy;
  permissionHandler?: AcpKimiPermissionHandler;
  signal?: AbortSignal;
...
export type AcpRuntimeOptions = Omit<AcpRuntimeDeps, "buildSpawnConfig"> &
  Partial<Pick<AcpRuntimeDeps, "buildSpawnConfig">>;
...
export async function createAcpSessionRuntime(
  deps: AcpRuntimeDeps,
): Promise<AcpSessionRuntime> {
  const cwd = deps.cwd ?? process.cwd();
  const env = deps.env ?? process.env;
  const spawnConfig =
    deps.buildSpawnConfig(env);
  const startupTimeoutMs = resolveStartupTimeoutMs(env);
  let emit = deps.emit ?? (() => {});
  let permissionHandler = deps.permissionHandler;
  const requestPermission: AcpKimiPermissionHandler = (params) => permissionHandler?.(params)
    ?? Promise.resolve({ outcome: { outcome: "cancelled" } });

  throwIfAborted(deps.signal);
  acpRuntimeLogger.info({
    cwd,
    command: spawnConfig.command,
    args: spawnConfig.args,
  }, "ACP_RUNTIME CREATE START");
  ...
  try {
      ...
      ? await runWithTimeout({
          ...
          task: async () => {
            ...
            if (!loaded || typeof loaded !== "object") {
              throw new AcpRuntimeError("ACP_SESSION_NOT_FOUND", "adapter.acp.session", "Agent could not load the saved session.");
            }
            return { ...loaded, sessionId: deps.sessionId! };
          },
        })
      : await runWithTimeout({
    ...
    }, deps.sessionId ? "ACP_RUNTIME LOAD_SESSION OK" : "ACP_RUNTIME NEW_SESSION OK");

    return {
      sessionId: session.sessionId,
      async prompt(input) {
        emit = input.emit;
        permissionHandler = input.permissionHandler ?? deps.permissionHandler;
        acpRuntimeLogger.info({
          sessionId: session.sessionId,
          messageLength: input.message.length,
        }, "ACP_RUNTIME PROMPT START");
    ...
    acpRuntimeLogger.error({
      cwd,
      errorMessage: error instanceof Error ? error.message : String(error),
    }, "ACP_RUNTIME CREATE ERROR");
    await closeConnection();
    throw error;
  }
}
...
export async function runAcpSingleTurn(
  input: AcpKimiChatRequest,
  deps: AcpRuntimeDeps,
): Promise<void> {
  const emit = deps.emit ?? (() => {});
  const runtime = await createAcpSessionRuntime({
    ...deps,
    emit,
  });
...
export async function createDefaultConnection(
  ...
  const collectingClient = createAcpCollectingClient(
    input.emit,
...
async function runPromptWithAbort(
  ...
  return await new Promise<{ stopReason: string }>((resolve, reject) => {
    const handleAbort = () => {
      acpRuntimeLogger.warn({
        sessionId,
      }, "ACP_RUNTIME PROMPT_ABORT");
      void Promise.resolve(connection.cancelSession?.({ sessionId }))
        .catch(() => {})
        .then(() => closeConnection ? closeConnection() : connection.close?.())
        .catch((error) => {
          acpRuntimeLogger.error({ sessionId, errorMessage: String(error) }, "ACP_RUNTIME ABORT_CLOSE_ERROR");
        });
      reject(abortError());
    };
...
function createCloseOnce(connection: AcpConnection): () => Promise<void> {
  let closing: Promise<void> | undefined;
  // Abort and finally may close concurrently. Both must await the same cleanup,
  // including child exit and its final audit, before reporting completion.
...
function createProcessStartupGuard(
  ...
  const handleStartError = (error: Error) => {
    ...
    startupSettled = true;
    startupError = new Error(`failed to start ACP agent: ${error.message}`);
    resolveStartup();
  };
...
function createProcessExitedError(
  ...
  return new AcpRuntimeError(
    "ACP_PROCESS_EXITED",
    "adapter.acp.process",
    `ACP process exited during ${operation} with code=${exitInfo?.code ?? "null"} signal=${exitInfo?.signal ?? "null"}.`,
  );
}
...
export function createAcpCollectingClient(
  emit: (event: AcpKimiStreamEvent) => void,
  permissionHandler?: AcpKimiPermissionHandler,
  capabilityPolicy?: AcpKimiClientCapabilityPolicy,
): CollectingClient {
...
function normalizeSessionSummary(session: Record<string, unknown>): AcpSessionSummary {
  const sessionId =

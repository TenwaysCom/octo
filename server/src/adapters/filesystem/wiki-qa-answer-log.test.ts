...
import { createWikiQaAnswerLog } from "./wiki-qa-answer-log.js";
...
function fixture(enabled?: string) {
  ...
  const env = { WIKI_QA_ANSWER_LOG_ENABLED: enabled };
  const write = createWikiQaAnswerLog({ env, createLogger: createLogger as unknown as typeof createFileLogger });
  return { write, sink, env, createLogger };
}

describe("wiki answerion diagnostic log", () => {
  ...
  it("uses a separate info-level sink and redacts embedded data on both sides", () => {
    ...
    expect(f.createLogger).toHaveBeenCalledExactlyOnceWith("./logs/wiki-qa-answer.log", "info", expect.any(Function));
    for (const [record] of f.sink.info.mock.calls) {
      expect(record).toMatchObject({ actionRunId: "run", stage: "server.wiki_qa.answer" });
    ...
    f.env.WIKI_QA_ANSWER_LOG_ENABLED = "false";

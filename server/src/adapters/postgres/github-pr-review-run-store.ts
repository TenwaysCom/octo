...
export interface GitHubPrReviewRun {
  actionRunId: string;
  masterUserId: string;
  operation: "github.pr.quick_scan" | "github.pr.deep_review";
  prUrl: string;
...
export interface GitHubPrReviewRunStore {
  ...
  markSucceeded(input: Pick<GitHubPrReviewRun, "actionRunId" | "commentUrl" | "reviewedFiles" | "diffTruncated">): Promise<void>;
  markFailed(input: Pick<GitHubPrReviewRun, "actionRunId" | "errorCode" | "errorMessage">): Promise<void>;
}
...
export class PostgresGitHubPrReviewRunStore implements GitHubPrReviewRunStore {
  ...
  async markSucceeded(input: Pick<GitHubPrReviewRun, "actionRunId" | "commentUrl" | "reviewedFiles" | "diffTruncated">): Promise<void> {
    const now = new Date().toISOString();
    await this.database.updateTable("github_pr_review_runs").set({
      ...
      diff_truncated: input.diffTruncated, completed_at: now, updated_at: now,
    }).where("action_run_id", "=", input.actionRunId).execute();
  }

  async markFailed(input: Pick<GitHubPrReviewRun, "actionRunId" | "errorCode" | "errorMessage">): Promise<void> {
    const now = new Date().toISOString();
    await this.database.updateTable("github_pr_review_runs").set({

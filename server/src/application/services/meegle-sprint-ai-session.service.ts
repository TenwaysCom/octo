...
export class MeegleSprintAiSessionError extends Error {
  constructor(
    readonly code: "SPRINT_NOT_FOUND" | "SESSION_NOT_FOUND" | "SESSION_FORBIDDEN" | "AI_ACTION_NOT_FOUND",
    message: string,
...
export function createMeegleSprintAiSessionService(deps: MeegleSprintAiSessionServiceDeps = {}) {
  ...
  return {
    ...
    async chat(input: {
      ...
      const quickAction = input.sessionId || !input.actionKey ? undefined : getSprintAiAutomationAction(input.actionKey);
...
async function buildSprintPrompt(
  promptStore: WorkflowPromptStore,
  quickAction: SprintAiAutomationActionConfig | undefined,
  context: SprintReleaseNotesContext,
  message: string,
) {
  const promptKey = quickAction?.promptKey ?? "meegle.sprint.release_notes";
  const template = (await promptStore.getByKey(promptKey))?.prompt.trim()
    || DEFAULT_MEEGLE_SPRINT_RELEASE_NOTES_PROMPT_TEMPLATE;

...
export function compactWikiAnswerContext(text: string): string {
  ...
  // Preserve referenced messages transitively; sparse M numbers still point at
  // the fixed original snapshot, not at a renumbered summary.
  let added: boolean;
  do {
    added = false;

  ...
  try {
    ...
    if (shadowEnabled) {
      if (shadowMasterUserId) {
        const shadowSummaryTimeoutSeconds = shadowTask.summaryTimeoutSeconds
          ?? shadowTask.deepSeekTimeoutSeconds
          ?? shadowTask.acpTimeoutSeconds;
        const shadowService = createLarkTicketShadowSummaryService({
          ...
          ...(shadowSummaryTimeoutSeconds ? { summaryTimeoutMs: shadowSummaryTimeoutSeconds * 1000 } : {}),
        });
